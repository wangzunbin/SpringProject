<template>
  <a-modal
    ...
    on-ok="handleOk"
  >
    <div v-if="$store.state.clientsModule.updateError">
      <a-alert
        style="width:100%"
        message="修改客户端失败"
        v-bind:description="$store.state.clientsModule.updateError"
        type="error"
      />
    </div>
    <a-form-model ref="form" :model="model" :rules="rules" v-bind="layout">
      ...
      <a-form-model-item has-feedback prop="redirectUris" label="重定向 URI">
        <a-input v-model="redirectUris" type="text" autocomplete="off" />
      </a-form-model-item>
...
<script>
import { editClientRules } from "@/views/form-rules/edit-client";
export default {
  props: {
    show: Boolean,
    record: Object,
  ...
  computed: {
    rules: function() {
      return editClientRules(this.model);
    },
    visible: {
      get() {
        return this.show;
    ...
    redirectUris: {
      get() {
        return this.model.redirectUris ? this.model.redirectUris.join(",") : [];
      },
      set(value) {
        this.model.redirectUris = value.split(",");
      },
    },
  },
  methods: {
    handleOk() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          this.$store
            .dispatch("clientsModule/update", this.model)
            .then((res) => {
              if (res) {
