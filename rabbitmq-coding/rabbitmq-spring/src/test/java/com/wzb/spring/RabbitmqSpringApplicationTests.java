...
class RabbitmqSpringApplicationTests {
    ...
    public void testAdmin() throws Exception {
        ...
        rabbitAdmin.declareBinding(BindingBuilder.bind(new Queue("test.topic.queue", false))  //直接创建队列
                .to(new TopicExchange("test.topic", false, false)).with("user.#"));  // 直接创建交换机, 简历关联关系
    ...
    //
    // 测试Jackson2JsonMessageConverter
    @Test
    ...
    public void testSendJavaMessage() throws Exception {
        ...
        //这里注意一定要修改contentType为 application/json
        messageProperties.setContentType("application/json");
        messageProperties.getHeaders().put("__TypeId__", "com.wzb.spring.entity.Order");
        Message message = new Message(json.getBytes(), messageProperties);

        rabbitTemplate.send("topic001", "spring.order", message);
    }

    // 对应的是DefaultJackson2JavaTypeMapper & Jackson2JsonMessageConverter 支持java对象转换
    @Test
